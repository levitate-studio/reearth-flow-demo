...
const MergeFeatures: LvtNodeDef = {
  _id: "MergeFeatures",
  ui: {
    title: "MergeFeatures",
    description:
      "Accept Featrue or FeatureCollection and merge into a array of Feature",
  },
  portsIn: [
    {
      name: "objects1",
  ...
  rule: (
    ...
    let result: any = [];
    for (let i = 0, m = res.length; i < m; i += 1) {
      for (let j = 0, n = res[i].length; j < n; j += 1) {
        if (res[i][j] && res[i][j].type === "FeatureCollection") {
          result = [...result, ...res[i][j].features];
        } else {
